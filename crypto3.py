...
def encrypt_file(input_filename, password, output_filename):
    salt = os.urandom(16)
    ...
    ciphertext_with_iv_salt = salt + iv + ciphertext

    try:
        with open(output_filename, "wb") as outfile:
            outfile.write(ciphertext_with_iv_salt)
        print(f"File '{input_filename}' encrypted and saved as '{output_filename}'.")
    except OSError as e:
        print(f"Error writing to output file: {e}")
        return

def decrypt_file(input_filename, password, output_filename):
    try:
        with open(input_filename, "rb") as infile:
            ciphertext_with_iv_salt = infile.read()
    except FileNotFoundError:
        print(f"Error: Input file '{input_filename}' not found.")
        return
    except OSError as e:
    ...
    salt = ciphertext_with_iv_salt[:16]
    iv = ciphertext_with_iv_salt[16:32]
    ciphertext = ciphertext_with_iv_salt[32:]
    key = generate_key_from_password(password, salt)
    cipher = Cipher(algorithms.AES(key), modes.CBC(iv), backend=default_backend())
...
if __name__ == "__main__":
    if len(sys.argv) < 5:
        print("Usage: python crypto.py <encrypt/decrypt> <input_file> <password> <output_file>")
        sys.exit(1)
